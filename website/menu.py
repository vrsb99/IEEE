...
def homepage(store_id):
    ...
    if request.method == 'POST':
        ...
        for product in products:
            if value := data.get(str(product.id)):
                new_order = Orders(products_id=product.id, quantity=int(value), store_id=store_id)
    ...
    return render_template("menu.html", store=store, categories=categories, products=products, images=images)
